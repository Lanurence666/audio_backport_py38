...
def _load_with_soundfile(
    ...
    if frame_offset < 0:
        frame_offset = 0
    if num_frames < 0:
        num_frames = frames - frame_offset
    data, sr = sf.read(uri, start=frame_offset, frames=num_frames, dtype='float32')
...
def load(
    ...
    if _TORCHCODEC_AVAILABLE:
        ...
        import warnings
        warnings.warn(
            "torchcodec is not available. Falling back to soundfile backend.",
            UserWarning,
            stacklevel=2,
        )
